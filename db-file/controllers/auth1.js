...
exports.researcherLogin = async(req,res)=>{
   try{
      ...
      db.query('SELECT * FROM organizationresearcher WHERE email = ?', [email], async(error,results)=>{
         console.log(results);
         if (results.length==0) {
...
exports.researcherSignup = (req, res) =>{
   ...
   const { name, email, organization, password, passwordConfirm }= req.body;
   ...
   db.query('SELECT email FROM organizationresearcher WHERE email = ?',[email], async(error, results) =>{
      ...
      db.query('INSERT INTO organizationresearcher SET ?',{name:name, email:email, organization:organization, password:hashedPassword},(error,results) =>{
         if(error){
            console.log(error);
         }
         else{
